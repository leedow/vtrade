...
describe('Kline事件驱动下：测试exchangeP usdt合约，自定义价格成交规则强制成交(修改order price)',function(){
  ...
  it('初始化exchange及资产账户',function(){
    exchange = new Exchange({
      ...
      limitOrderFillPriceRule: (ex, order) => {
        order.price *= 2
        return {
          buyPrice: ex.getBidPrice(), 
          sellPrice: ex.getAskPrice()
        }
      }
    })
  ...
  it('测试仓位价格',function(){
    assert.equal( exchange.long.avgPrice.toFixed(2), ((5001*2+4999*1)*2/3).toFixed(2) )
